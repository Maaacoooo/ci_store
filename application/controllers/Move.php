...
class Move extends CI_Controller {
	...
	public function index()		{
		...
		if($userdata)	{
			...
			//Paginated data				            
	   		$config['num_links'] = 5;
			$config['base_url'] = base_url('/move/index/');
			$config["total_rows"] = $this->move_model->count_move($search);
			$config['per_page'] = 20;				
		    ...
		    $data["results"] = $this->move_model->fetch_move($config["per_page"], $page, $search);
				...
				if($move_id) {				
					...
					if($this->input->post('destination')) {
						...
						$log[] = array(
							...
							'tag' 		=> 	'location',
							'tag_id'	=> 	$id,
							'action' 	=> 	'Disposed Items - MOVE #'.prettyID($move_id)
						);
					}
	...
	public function view($id)		{
		...
		if($userdata)	{
			...
			$data['info']		= $this->move_model->view($id);
			...
			if($data['user']['usertype'] == 'Administrator') {
				//View		
				$data['items'] = $this->move_model->fetch_move_items(NULL, NULL, $id);
				
				$data['title'] = 'Move #'.prettyID($data['info']['id']);						
				$this->load->view('move/view', $data);	
